...
Os dashboards versionados `19924.json`, `19925.json` e `orderbook-business.json`
identificam os dashboards Grafana 19924/19925 e o painel de negócio local. A fonte consultada foi a Grafana API
...
O datasource Tempo configura o vínculo trace-to-logs usando o recurso
`service.name` como label Loki `service_name` e filtra o `trace_id` no conteúdo
JSON do log. O Trace ID não é usado como label Loki, evitando cardinalidade
excessiva.
...
`orders_received_total` usa somente a tag de baixa cardinalidade `side=BUY|SELL`.
